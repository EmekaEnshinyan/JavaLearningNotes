...
public class ImageProcessing {
   ...
   twoDToImage(negColor, "./negative_apple.jpg");
    int stretchH[][] = stretchHorizontally(imageData);
    twoDToImage(stretchH, "./stretched_apple.jpg");
    ...
    for (int i = 0; i < imageTwoD.length; i++){
      for (int j = 0; j < imageTwoD[i].length; j++){
        it = j * 2;
        manipulatedImage[i][it] = imageTwoD[i][j];
        manipulatedImage[i][it + 1] = imageTwoD[i][j];
